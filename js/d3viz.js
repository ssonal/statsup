var plot = function(data){
	...
	function initConvoPlot() {

		var data1 = []
		var dataLength = data[names[0]]['initiated_per_day'].length

		_.each(_.range(dataLength), function(i) {
			data1[i] = _.map(names, function(name){
				return { name:name, value:+data[name]['initiated_per_day'][i] }
			})
		})
		...
		var svg = d3.select("#initConvoAnalysis")
			.append("svg")
				.attr("width", width+margin.left+margin.right)
				.attr("height", height+margin.top+margin.bottom+30)
				.append("g")
		...
		var sumY = []
		maxY = _.max(_.map(maxY, function(d) {sumY.push(d3.sum(d));return _.max(d)}))
		// console.log(data1)
		// console.log(sumY)
		// console.log(maxY)
		x0.domain(_.range(dataLength))
