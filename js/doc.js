var preProcess = function(data) {
    ...
    if (dateCheck.length != 1) {
        ...
        if (!moment(data[0].match(/^.*[APM]? (?=-)/)[0].split(',').join('').trim().split(' '), dateFormat).isValid()) {
    ...
    var msgs = _.map(data, function(line) {
        var time = line.match(/^.*? (?=-)/)[0].split(',').join('').trim().split(' ');

        // handles cases when the year is missing from messages by inserting present year
        if (time.length == 4)
            time.splice(2, 0, new Date().getFullYear().toString());
        time = moment(time, dateFormat)

        try {
            var name = line.match(/- ([\w ]+(?=:))/)[1].trim();
        } catch (e) {
            return null;
        }
        var message = line.match(/: (.*)/)[1].trim();

        return {'time' : time['_d'], 'name' : name, 'message' : message};
    });
    ...
    _.each(people, function(person) {
        jsonData[person] = {
            'hourly': math.zeros(24).toArray(),
            'daily': math.zeros(dateDiff(_.last(msgs)[0], msgs[0][0]) + 1).toArray(),
            'initiated_per_day': math.zeros(dateDiff(_.last(msgs)[0], msgs[0][0]) + 1).toArray(),
